...
export const IncidentDrawer = ({
    ...
    const incidentStatus = useMemo(() => selectedIncident?.incidentStatus, [selectedIncident])
        useEffect(() => {
            if (incidentStatus) {
                setUpdate(prev => ({
                    ...prev,
    ...
    const handleSaveIncident = async () => {
        addIncident({ ...selectedIncident, incidentStatus: selectedIncident?.incidentStatus || 'Investigating' }, handleDrawerClose)
    }
    console.log('services draww', services, update)
    return (

        <>
            <Drawer placement="right" onClose={handleDrawerClose} isOpen={isDrawerOpen} size="md">
                <DrawerOverlay />
                <DrawerContent>
                                    ...
                                    {services &&
                                        services.map((service) => (
                                            <Box key={service.serviceId}>
                                                <Checkbox
                                                    isChecked={!!selectedIncident?.affectedServicesIds?.find(
                                                    ...
                                                    onChange={(e) =>
                                                        setSelectedIncident((prev) => ({
                                                            ...
                                                            affectedServicesIds: e.target.checked
                                                                ? [...(prev.affectedServicesIds || []), service.serviceId]
                                ...
                                render={(props) => (
                                    ...
                                    >
                                        ...
                                        <VStack spacing="24px">
                                            ...
                                            <FormControl id="updateAffectedServicesStatus">
                                                <Button isLoading={props.loadingAdd} onClick={() => props.addResource(update,addUpdateNext)}>Post Update</Button>
                                            </FormControl>
                                            <Divider />
                                            {props.data?.updates?.sort((a, b) => b.updateCreatedAt - a.updateCreatedAt)
                                            .map(update => (
                                                <HStack
                                                ...
                                                >
                                                    ...
                                                    <UpdatedAt updatedAt={update.updateUpdatedAt} createdAt={update.updateCreatedAt}/>
                                                        ...
                                                        {props.loadingDelete === update.updateId ? <Spinner  size="xs" margin="auto 5px"/> : <DeleteIcon size="xs"  marginLeft="5px" cursor="pointer" onClick={() => props.deleteResource(update.updateId)} />}
                                                    </HStack>
                ...
                >
                    <AlertDialogOverlay>
                        <AlertDialogContent>
                            ...
                            <AlertDialogBody>
                                Are you sure you want to delete <Text as="b">{selectedIncident?.incidentName}</Text> ?
                            </AlertDialogBody>
                            ...
                            <AlertDialogFooter>
                                ...
                                <Button isLoading={loadingDelete} colorScheme="red" onClick={() => deleteIncident(selectedIncident?.incidentId, deleteIncidentNext)} ml={3}>
                                    Delete
                                </Button>
                            </AlertDialogFooter>
                        </AlertDialogContent>
